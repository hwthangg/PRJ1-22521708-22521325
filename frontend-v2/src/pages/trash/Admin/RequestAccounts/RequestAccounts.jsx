...
import RequestAccountDetails from "./RequestAccountDetails/RequestAccountDetails";
...
function RequestAccounts() {
  ...
  useEffect(() => {
    const fetchAccounts = async () => {
      const res = await fetch(
        `http://localhost:5000/api/accounts?page=${currentPage}&limit=7&search=${search}&status=waiting&role=${role}&sortBy=createdAt&sortOrder=asc`,
        {
          method: "GET",
          headers: {
  ...
  return (
    <div style={styles.container}>
      ...
      {openDetails && <RequestAccountDetails id={id} setOpen={setOpenDetails} />}
    </div>
  );
}
